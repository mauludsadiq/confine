...
`ports/rust`, `ports/python`, and `ports/js` implement sections 2–8 of that spec (canonical encoding, tagged hashing, HMAC, label lattice, capability model) and are verified against real vectors captured by running the actual FARD implementation via `fardrun` — not derived from reading the specification alone. This caught a real bug during the Rust port: `certificate.fard`'s broker secret is normalized via `bytes.to_hex(bytes.of_str(secret))` before being passed to `hmac_sha256` (which hex-decodes it again), so the effective HMAC key is the raw UTF-8 bytes of the secret string, not a hex-decode of it — a one-character assumption that would have silently broken cross-language certificate verification if a failing test hadn't caught it first.
...
Sections 9 onward (`obligations_for_action`, the full `policy.verify` decision procedure) are not yet ported — they require realistic state (invoices, drafts, approvals) as input rather than isolated pure functions, and will need vectors generated by walking the actual `create_draft` -> `approve_draft` -> `submit_draft` flow.
